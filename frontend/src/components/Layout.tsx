        ...
        <div className="fixed inset-y-0 left-0 flex w-64 flex-col bg-white">
          ...
          <nav className="flex-1 overflow-y-auto p-4">
            {filteredNavigation.map((item) => {
              ...
              const isDisabledByTest = isTestInProgress && !location.pathname.startsWith(item.href)
              return (
                <Link
                  key={item.name}
                  to={isDisabledByTest ? '#' : item.href}
                  onClick={(e) => {
                    if (isDisabledByTest) { e.preventDefault(); return }
            ...
            {filteredNavigation.map((item) => {
              ...
              return (
                <Link
                  key={item.name}
                  to={isDisabledByTest ? '#' : item.href}
                  onClick={isDisabledByTest ? (e: React.MouseEvent) => e.preventDefault() : undefined}
                  title={isDisabledByTest ? 'Cannot navigate during active test' : undefined}
                  className={clsx(
                    'flex items-center px-4 py-3 mb-1 rounded-lg text-sm font-medium transition-colors',
                    isDisabledByTest
        ...
        <header className="sticky top-0 z-30 bg-white border-b border-gray-200">
          <div className="flex h-16 items-center justify-between px-4 sm:px-6 lg:px-8">
            ...
            <div className="flex items-center gap-4">
              {/* Notifications */}
              {isTestInProgress ? (
                <span className="relative p-2 text-gray-300 cursor-not-allowed" title="Cannot access during active test">
                    ...
                    <div className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-gray-200 py-1 z-50">
                      ...
                      {isTestInProgress ? (
                        <span
                          ...
                          title="Cannot logout during active test"
                        >
